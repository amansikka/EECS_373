...
int main(void)
{
  ...
  while (1)
  {
	...
	if (HAL_UART_Receive(&huart6, &rxChar, 1, 100) == HAL_OK)
	{
	  if (rxChar == '\n')
	  {
		rxBuf[rxIndex] = '\0';
		if (sscanf(rxBuf, "%d,%d,%d", &dir, &speed, &boost) == 3)
		{
		  // Handle boost
		  if (boost == 1)
		  {
		  ...
		  // Handle direction & speed
		  if(dir == 0) // Forward
		  {
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, GPIO_PIN_SET);
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_9, GPIO_PIN_SET);
			SetDutyCycleLeft(speed);
			SetDutyCycleRight(speed);
		  }
		  else if(dir == 1) // Left
		  {
