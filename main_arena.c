...
uint8_t IMU_Direction(int16_t x)
{
  if (x > 3000)
  {
	return 1; // left
...
uint16_t Flex_Magnitude(uint32_t x)
{
...
int main(void)
{
  ...
  while (1)
  {
	...
	bMag = 50;
	int bKick = 1;
	printf("Transmitting (string): %s", txBuf);
	sprintf(txBuf, "%d,%d,%d\r\n", (int)bDir, (int)bMag, bKick);

	HAL_UART_Transmit(&huart3, (uint8_t*)txBuf, strlen(txBuf), 250);
	HAL_Delay(200);
    /* USER CODE END WHILE */
