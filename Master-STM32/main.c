...
uint32_t Flex_Read(void)
{
  HAL_ADC_Start(&hadc1);
  bFlexRaw = HAL_ADC_GetValue(&hadc1);
  bFlexRaw = ((bFlexRaw * 100) / ADC_MAX_VALUE);
  bMag = Flex_Magnitude(bFlexRaw);

  HAL_Delay(50);

  yFlexRaw = HAL_ADC_GetValue(&hadc1);
  yFlexRaw = ((yFlexRaw * 100) / ADC_MAX_VALUE);
  yMag = Flex_Magnitude(yFlexRaw);

  //new code

  if (bFlexRaw < RAW_MIN) bFlexRaw = RAW_MIN;
  if (bFlexRaw > RAW_MAX) bFlexRaw = RAW_MAX;


  bFlexRaw = ((bFlexRaw - RAW_MIN) * (SCALE_MAX - SCALE_MIN)) / (RAW_MAX - RAW_MIN) + SCALE_MIN;
